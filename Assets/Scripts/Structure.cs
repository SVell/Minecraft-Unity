...
{
    public static Queue<VoxelMod> GenerateMajorFlora(int index, Vector3 position, int minTrinkHeight,
        int maxTrunkHeight)
    {
        switch (index)
        {
            case 0:
                return MakeTree(position, minTrinkHeight, maxTrunkHeight);
            case 1:
                return MakeCactus(position, minTrinkHeight, maxTrunkHeight);
        }
    ...
    public static Queue<VoxelMod> MakeCactus(Vector3 position, int minTrinkHeight, int maxTrunkHeight)
    {
        Queue<VoxelMod> queue = new Queue<VoxelMod>();
        int height = (int) (maxTrunkHeight * Noise.Get2DPerlin(new Vector2(position.x, position.y), 33333, 4f));
        if (height < minTrinkHeight)
            height = minTrinkHeight;
